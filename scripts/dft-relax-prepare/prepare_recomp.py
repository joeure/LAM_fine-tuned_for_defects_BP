...
MAP = {
    "A1": "INCAR_RECOMP_A1.jinja",
    "A2": "INCAR_RECOMP_A2.jinja",
    "B1": "INCAR_RECOMP_B1.jinja",
    "B2": "INCAR_RECOMP_A2.jinja",   # reuse
    "C":  "INCAR_RECOMP_C.jinja",
}

def choose_plan(reason_codes, fmax):
    rc = set(reason_codes.split(";"))
    # Bucket C (near done but stuck)
    if "stagnated_no_progress" in rc or (0.010 < fmax <= 0.020):
        return "C"
    # Bucket B (oscillations)
    if "energy_oscillation" in rc:
        return "B1"
    # Bucket A (big forces)
    if fmax > 0.06:
        return "A1"
    # Medium forces, no oscillation → go straight to A2
    return "A2"

def choose_plan_for_sys(cif_id, df, templates_dir):
    print(f"cif: {cif_id}, in: {cif_id not in df['cif_id']}")
    if cif_id not in df["cif_id"].to_list():
        return None
    row = df[df["cif_id"] == cif_id].iloc[0]
    if row["status"] == "converged":
        return None
    fmax = float(row["Fmax_last_step_eVA"] or "0")
    plan = choose_plan(row["reason_codes"], fmax)
    return os.path.join(templates_dir, MAP[plan])
...
def prepare_one_system(
    ...
    if recompCsv is not None:
        print(systemID)
        incarTemplate = choose_plan_for_sys(systemID, recompCsv, templatePath)
        if incarTemplate is None:
            print(f"discarded: {incarTemplate} for {systemID}")
            return systemPath
    ...
    if recompCsv is not None:
        contcar = os.path.join(former_ret, systemID, "CONTCAR")
        chgcar = os.path.join(former_ret, systemID, "CHGCAR")
        afterchgPath = os.path.join(systemPath, "CHGCAR")
        if os.path.exists(contcar):
            shutil.copy2(contcar, poscarPath)
            shutil.copy2(chgcar, afterchgPath)
        else:
            raise FileNotFoundError(f"File Not Found: {contcar}")
    else:
...
def prepare_batch_inputs(
    ...
    recompCsv = pd.read_csv(recomp_csv)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepare VASP recomputation inputs for systems selected from a prior convergence report."
    )
    ...
    parser.add_argument(
        "--prepared-path",
        required=True,
        help="Output directory for generated recomputation folders.",
    )
    parser.add_argument(
        "--template-path",
    ...
    parser.add_argument("--mask-csv", default=None, help="Optional CSV used to mask systems.")
    parser.add_argument(
        "--recomp-csv",
        required=True,
        help="CSV listing systems selected for recomputation, e.g. unconverged_report.csv.",
    )
    parser.add_argument(
        "--former-ret",
        required=True,
        help="Directory containing former VASP relaxation outputs used as recomputation references.",
    )
...
if __name__ == "__main__":
    ...
    prepare_a_batch_system(
        ...
        recomp_csv=args.recomp_csv,
        former_ret=args.former_ret,
    )
