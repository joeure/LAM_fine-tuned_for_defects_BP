...
def calculate(input_script, log_file):
    ...
    for attempt in range(1, tries + 1):
        ...
        try:
            ...
            # Optional: confirm pair/min styles available
            lmp.command("info styles pair")
            ...
            # DeepMD GPU OOMs still manifest as cudaMalloc inside the DeepMD op
            if "cudaMalloc" in str(err):
                logging.warning("⚠️ DeepMD GPU OOM, retry after %ds …", delay)
            else:
                logging.error("❌ %s (retry in %ds)", err, delay)
            time.sleep(delay)
    else:
        raise RuntimeError(f"gave up after {tries} tries")
...
def transfer_data_files(parentDir, lowHigh:bool, system:str, data_type:DataType, outputParent:str, refSystem:str=None):
    ...
    if data_type != DataType.REFERENCE:
        ...
        filesDF = pd.read_csv(os.path.join(".", f"{data_type.value}.csv"))
...
def benchmarking(parentDir:str, outputParent:str, calculateSystem: SystemDefinition, 
    ...
    if set(csvTrain['atoms_id']).intersection(set(csvVal['atoms_id'])):
        ...
        csvTrain, csvVal = preprocess(csvFile1=csvFileTrain, csvFile2=csvFileVal, outputFile=os.path.join(parentDir, "low_density_defects" if calculateSystem.lowHighDensity else "high_density_defects",
